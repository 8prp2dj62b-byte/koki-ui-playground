...
export function classifyRemoteListing(ad:any):ExternalListingState{
  ...
  if(ad.is_active===false||ad.active===false||['inactive','archived','disabled','removed','sold','expired','closed'].includes(s))return'INACTIVE';
...
async function pauseSellAutomation(accountId:string,identity:{advert_id:number|null,external_id:string|null},externalState:'INACTIVE'|'DELETED_EXTERNAL',now:string,supa:Supa){
  const rr=await supa(`koki_olx_thread_runtime?olx_account_id=eq.${encodeURIComponent(accountId)}&domain=eq.SELL&select=thread_id,domain,runtime_mode,metadata`);if(!rr.ok)return 0;
  const rows=await json(rr),all=Array.isArray(rows)?rows:[],matched=all.filter((row:any)=>{const rid=identityOf(row?.metadata||{}),sameId=!!identity.advert_id&&rid.advert_id===identity.advert_id,sameExt=!!identity.external_id&&rid.external_id===identity.external_id;return sameId||sameExt}),tids:number[]=[];
  for(const row of matched){const tid=Number(row.thread_id);if(!Number.isFinite(tid))continue;tids.push(tid);await supa(`koki_olx_thread_runtime?olx_account_id=eq.${encodeURIComponent(accountId)}&thread_id=eq.${tid}`,{method:'PATCH',body:JSON.stringify({domain:'SELL',runtime_mode:'SLEEPING',next_reconcile_at:null,metadata:{...(row.metadata||{}),external_listing_state:externalState,external_advert_id:identity.advert_id,external_id:identity.external_id,external_listing_changed_at:now},updated_at:now})}).catch(()=>{})}
  if(tids.length){const cr=await supa(`koki_sell_conversations?thread_id=in.(${tids.join(',')})&select=id,thread_id,buyer_intelligence,control_owner`);if(cr.ok){const convs=await json(cr);for(const c of(Array.isArray(convs)?convs:[]))await supa(`koki_sell_conversations?id=eq.${encodeURIComponent(c.id)}`,{method:'PATCH',body:JSON.stringify({control_owner:'PAUSED',buyer_intelligence:{...(c.buyer_intelligence||{}),external_listing_state:externalState,external_advert_id:identity.advert_id,external_id:identity.external_id,external_listing_changed_at:now},updated_at:now})}).catch(()=>{})}}
  return tids.length;
}
...
export async function reconcileDeletedOwnListings(opts:{accountId:string;ownerProfileId?:string;supa:Supa;olx:Olx;audit:Audit;notify?:Notify;now?:string}){
  ...
  const listingRows=await json(lr),saleRows=sr.ok?await json(sr):[],sales=Array.isArray(saleRows)?saleRows:[],owned=(Array.isArray(listingRows)?listingRows:[]).filter((row:any)=>identityOf(row).advert_id&&belongsToAccount(row,sales,accountId,ownerProfileId)&&managedCandidate(row));
